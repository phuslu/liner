...
func (h *HTTPWebHandler) Load() (err error) {
	...
	h.handle, err = h.makeIndexHandler(h.Config.Web.Headers, h.Config.Web.Body)
	if err != nil {
		return
	}
...
func (h *HTTPWebHandler) makeIndexHandler(headers, body string) (httprouter.Handle, error) {
	htmpl, err := template.New(headers).Funcs(h.Functions).Parse(headers)
	if err != nil {
		return nil, err
	}

	btmpl, err := template.New(body).Funcs(h.Functions).Parse(body)
	if err != nil {
		return nil, err
	}

	addHeaders := func(rw http.ResponseWriter, req *http.Request, params httprouter.Params) {
		var sb strings.Builder
		htmpl.Execute(&sb, struct {
			WebRoot   string
	...
	return func(rw http.ResponseWriter, req *http.Request, params httprouter.Params) {
		...
		if h.Config.Web.Root == "" {
			addHeaders(rw, req, params)
			btmpl.Execute(rw, struct {
				WebRoot   string
				Request   *http.Request
				FileInfos []os.FileInfo
			}{h.Config.Web.Root, req, nil})
