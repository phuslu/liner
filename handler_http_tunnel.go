...
func (h *HTTPTunnelHandler) h2tunnel(rw http.ResponseWriter, req *http.Request, ri *HTTPRequestInfo, addrport netip.AddrPort, ln net.Listener) {
	var conn net.Conn
	var err error
	...
	if req.ProtoAtLeast(2, 0) {
		...
		if err != nil {
			log.Error().Err(err).Context(ri.LogContext).Str("username", ri.AuthUserInfo.Username).Msg("tunnel hijack request error")
			http.Error(rw, err.Error(), http.StatusInternalServerError)
			return
		}
		...
		_, err = conn.Write(b)
		if err != nil {
			log.Error().Err(err).Context(ri.LogContext).Str("username", ri.AuthUserInfo.Username).Msg("tunnel send response error")
			return
		}
	}
	...
	if err != nil {
		log.Error().Err(err).Context(ri.LogContext).Str("username", ri.AuthUserInfo.Username).Msg("tunnel open mux session error")
		http.Error(rw, err.Error(), http.StatusInternalServerError)
		return
	}
