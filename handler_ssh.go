...
func NewSSHServer(shellPath string, config *ssh.ServerConfig, logger *log.Logger) *SSHServer {
	if logger == nil {
		logger = log.New(ioutil.Discard, "", 0)
	}
	return &SSHServer{shellPath: shellPath, config: config, logger: logger}
}
...
func (s *SSHServer) handleConn(tcpConn net.Conn) {
	// Before use, a handshake must be performed on the incoming net.Conn.
...
func (s *SSHServer) isClosed() bool {
	s.mu.Lock()
	closed := s.closed
	s.mu.Unlock()
	return closed
...
// start shell
func (s *SSHServer) startShell(shellPath string, connection ssh.Channel) *SSHShellFile {
