...
func (m *TLSInspector) GetConfigForClient(hello *tls.ClientHelloInfo) (*tls.Config, error) {
	...
	m.ClientHelloMap.Store(PlainAddrFromAddrPort(addr), &TLSClientHelloInfo{ClientHelloInfo: hello})
...
func (m *TLSInspector) HTTP3ConnContext(ctx context.Context, conn *quic.Conn) context.Context {
	...
	if info, ok := m.ClientHelloMap.Load(PlainAddrFromAddrPort(addr)); ok {
		...
		m.ClientHelloMap.Delete(PlainAddrFromAddrPort(addr))
	}
