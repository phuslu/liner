...
func (h *HTTPWebIndexHandler) Load(ctx context.Context) (err error) {
	...
	h.headers, err = h.Functions.ParseTemplate("http_web_index_headers", h.Headers)
	if err != nil {
		return
	}

	h.body, err = h.Functions.ParseTemplate("http_web_index_body", h.Body)
	if err != nil {
		return
	}
	...
	// }
	h.markdown, err = h.Functions.ParseTemplate("http_web_index_markdown", markdown)
	if err != nil {
		return
	}
...
func (h *HTTPWebIndexHandler) ServeHTTP(rw http.ResponseWriter, req *http.Request) {
	...
	if h.Root == "" {
		...
		if h.File != "" {
			...
			tmpl, err = h.Functions.ParseTemplate("http_web_index_file", string(data))
			if err != nil {
